...
import { Dialog, DialogTrigger, DialogSurface, DialogTitle, Button, Spinner } from '@fluentui/react-components';
...
export const SignInDialog = () => {
  ...
  const { data, isLoading } = useQuery(['providers', prompt], () => getProviders(), {
        ...
        <div className={signInDialogStyles.signInButtonWrapper}>
          {!isLoading && data ? (
            Object.values(data).map((provider: any, index) => (
              <ProviderButton provider={provider} id={index} key={provider.id} />
            ))
          ) : (
            <Spinner />
          )}
